...
if( isset($_POST['sask']) ) {
	if( $_POST['sask']['saskaitos_suma'] != '' ) 
	{
		...
		if(!is_numeric($skaicius)){
			...
			header('Location: ./index.php?psl=nuskaiciuoti&id='.$id.'&status=2&message=17');
			
		}
			else{
			if($sask['saskaitos_suma'] < $_POST['sask']['saskaitos_suma']){
			
				header('Location: ./index.php?psl=nuskaiciuoti&id='.$id.'&status=2&message=10');
				
			}else{
				
				$data[$id]['saskaitos_suma'] = $sask['saskaitos_suma'] - $_POST['sask']['saskaitos_suma'];
				
				if( file_put_contents( $datafilelink, json_encode($data) ) ) {

					header('Location: ./index.php?psl=sarasas&status=1&message=11');
					
				} else {

					header('Location: ./index.php?psl=nuskaiciuoti&id='.$id.'&status=2&message=4');
	...
	<div class="col-lg-12">
		<form method="POST"  id="saskaitos_sukurimas">
			<div class="row g-3">
				...
				<div class="col-sm-4">
					<label class="form-label"><b>Įveskite sumą kurią norite išimti iš sąskaitos</b></label>
					<input type="text" id="saskaitos_suma" class="form-control" name="sask[saskaitos_suma]" value="<?php echo $sask['saskaitos_suma']; ?>" />
				</div>
			</div>
			<div class="mt-5 mb-5">
				<button class="w-100 btn btn-success btn-lg" type="submit">Išimti lėšas iš sąskaitos</button>
			</div>
		</form>
	</div>
</div>
